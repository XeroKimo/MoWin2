...
namespace MoWin
{
    ...
    struct RawWindowClassTraits;

    template<>
    struct RawWindowClassTraits<LPCSTR>
    {
        using standard_type = WNDCLASSA;
        using extended_type = WNDCLASSEXA;
        using string_type = LPCSTR;

        static WindowClassAtom Register(const standard_type& data)
        {
            return WindowClassAtom(RegisterClassA(&data));
        }

        static WindowClassAtom Register(const extended_type& data)
        {
    ...
    template<>
    struct RawWindowClassTraits<LPCWSTR>
    {
        using standard_type = WNDCLASSW;
        using extended_type = WNDCLASSEXW;
    ...
    template<>
    struct DefaultClass<LPCSTR> : public DefaultClassCommon
    {
        using string_type = LPCSTR;

        static string_type ClassName() { return "DefaultClassA"; }
        static string_type MenuName() { return nullptr; }

        static LRESULT Procedure(Event event)
        {
            return RawWindowClassTraits<string_type>::DefaultProcedure(event.window, static_cast<UINT>(event.type), event.wParam, event.lParam);
        }
    };
    ...
    template<class Ty, class RawTraits>
    struct BaseWindowClassTraits
    {
    public:
        using raw_traits = RawTraits;
        using class_type = raw_traits::extended_type;
        using string_type = raw_traits::string_type;

    private:
        inline static std::atomic<unsigned short> m_registeredCount;
        inline static HINSTANCE m_instance;

    public:
        static bool Registered()
        {
            return m_registeredCount.load() > 0;
        }
        ...
        static bool Register(HINSTANCE instance = nullptr)
        {
            ...
            if constexpr(IconDefined<Ty>)
                windowClass.hIcon = Ty::Icon(instance);
            else
                windowClass.hIcon = DefaultClass<string_type>::Icon(instance);

            if constexpr(CursorDefined<Ty>)
                windowClass.hCursor = Ty::Cursor(instance);
            else
                windowClass.hCursor = DefaultClass<string_type>::Cursor(instance);
            ...
            if constexpr(SmallIconDefined<Ty>)
                ...
                windowClass.hIconSm = DefaultClass<string_type>::SmallIcon(instance);
        ...
        {
            unsigned short count = m_registeredCount.load();
            if(count == 0)
                return;
    ...
    struct WindowClassTraits<Ty> : BaseWindowClassTraits<Ty, RawWindowClassTraits<LPCSTR>>
    {

    };

    template<IsWindowClassW Ty>
    struct WindowClassTraits<Ty> : BaseWindowClassTraits<Ty, RawWindowClassTraits<LPCWSTR>>
    {
    ...
    template<class CharTy>
    struct RawWindowTraits;

    template<>
    struct RawWindowTraits<LPCSTR>
    {
        using string_type = LPCSTR;

        static HWND Create(WindowStyleEx extendedStyle, string_type className, string_type windowName, WindowStyle style, int x, int y, int width, int height, HWND optionalParent, HMENU menu, HINSTANCE hInstance, void* data)
        {
    ...
    template<>
    struct RawWindowTraits<LPCWSTR>
    {
        using string_type = LPCWSTR;
    ...
    private:
        ...
        using window_traits = RawWindowTraits<string_type>;
